...
function showImageModal(src, pageName) {
  if (
    store.isImageModalVisible ||
    (pageName && pageName !== store.nav.currentPageName)
  ) {
    return;
  }
 ...
 * @param {*} onError
 */
function sendJoinRoomRequest(onResponse, onError) {
  let _joind = false;
...
function cacheMessageData(md5sum, dataUrl) {
  const msgDataCacheMap = store.cacheMap.msgData;
...
function hasMessageCache(messageInfo) {
  ...
  const hasCache =
    messageInfo.hasOwnProperty('dataID') &&
    msgDataCacheMap.hasOwnProperty(messageInfo.md5sum);
...
function hasDataCacheByMD5(md5sum) {
  ...
  const hasCache = msgDataCacheMap.hasOwnProperty(md5sum);
